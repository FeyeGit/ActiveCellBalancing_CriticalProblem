    ...
    if feedback == 0
        ...
        if k<length(P)
            ...
            while isreal(w_pred)==0 || cut_back==1
                ...
                %% Compute worst scenario
                if hor == 1
                    v_pred = VelSC10(:,1:hn,k);
                elseif hor == 2
                    v_pred = VelSC25(:,1:hn,k);
                else
                    v_pred = VelSC50(:,1:hn,k);
                end
                ...
                for j=1:20
                    for i=1:length(v_pred(1,:))-1
                        ...
                        P_pred(j,i) = -Ftract(i)*v_pred(i)/scaling;
                    end
                    P_tryout(j,:) = [P(k) P_pred(j,1:hn-1)];
                end
%                     clear P_tryout x_tryout y_tryout w_tryout  
%                     P_tryout(j,:) = [P(k) P_pred(j,1:hn-1)];
%                     
%                     x_tryout = x(:,k);
%                     for i=1:numel(P_tryout(j,:))
%                         for n = 1:N
%                             Af = A(x_tryout(2*n-1,i))^AB_var(n);
%                             Bf = B(x_tryout(2*n-1,i))*AB_var(n);
%                             As(2*n-1:2*n,2*n-1:2*n) = [1 0; 0 Af];
%                             Bs(2*n-1:2*n,1:n+1) = [1/(C0*C0_var(n)) zeros(1,n-1) 1/(C0*C0_var(n));Bf zeros(1,n-1) Bf];
%                             R0(n) = D(x_tryout(2*n-1,i))*D_var(n);
%                             Vemf(n) = EMF(x_tryout(2*n-1,i));
%                         end
% 
%                         a = sum(R0);
%                         b = sum(Vemf)+repmat([0 1],1,N)*x_tryout(:,i);
%                         c = -P_tryout(j,i);
%                         w_tryout(i) = (-b + sqrt(b^2-4*a*c))/(2*a);
%                         if isreal(w_tryout(i)) == 0
%                             w_tryout(i) = abs(w_tryout(i));
%                         end
% 
%                         if k == 1
%                             U = [w_tryout(i); zeros(10,1)];
%                         else
%                             U = [w_tryout(i); u_opt_MPC];
%                         end
% 
%                         x_tryout(:,i+1) = As*x_tryout(:,i) + Bs*U;
%                         for n=1:N
%                             y_tryout(n,i) = x_tryout(2*n,i) + [R0(n) zeros(1,n-1) R0(n) zeros(1,N-n)]*(U) + Vemf(n);
%                         end
%                     end
%                 end
...
tosave = strcat(savefolder,'worst_case_',name(method),'_',num2str(horizon));
